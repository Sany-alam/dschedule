...
if (isset($_POST['AddingSpecialty'])) {
    ...
    $query = "SELECT * FROM `tbl_specialty` WHERE name = `$specialty`";
    $confirm = mysqli_query($conn,$query);
    if ($confirm) {
        echo json_encode($confirm);
    }
    else{
        $query = "INSERT INTO `tbl_specialty`(`name`) VALUES ('$specialty')";
        $confirm = mysqli_query($conn,$query);
        if ($confirm) {
            echo "Added";
        }else{
...
if (isset($_POST['EditSpecialty'])) {
    $id = $_POST['id'];
    $query = "SELECT * FROM tbl_specialty WHERE id = '$id'";
    $result = mysqli_query($conn,$query);
...
if (isset($_POST['DeleteSpecialty'])) {
    $id = $_POST['id'];
    $query = "DELETE FROM `tbl_specialty` WHERE `id` = '$id'";
    $result = mysqli_query($conn,$query);
}
...
if (isset($_POST['UpdateSpecialty'])) {
    $id = $_POST['id'];
    $specialty = $_POST['specialty'];
    $query = "UPDATE `tbl_specialty` SET `name`='$specialty' WHERE `id` = '$id'";
    $result = mysqli_query($conn,$query);
}
?>