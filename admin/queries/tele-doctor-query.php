...
if (isset($_POST['AddingDoctor'])) {
    ...
    $password = "123456";
    $a = "SELECT * FROM `tbl_doctor_tele` WHERE `email` = '$email'";
    ...
    if (mysqli_num_rows($a_res)>0) {
        ...
        $query = "INSERT INTO `tbl_doctor_tele`(`name`, `email`, `password`, `msisdn`, `details`, `specialty_id`) VALUES ('$name','$email','$password','$msisdn','$detail','$specialty')";
...
if (isset($_POST['Show'])) {
    $query = "SELECT * FROM tbl_doctor_tele";
    $result = mysqli_query($conn,$query);
    ...
    while ($row=mysqli_fetch_array($result)) {
        ...
        $sp_query = "SELECT * FROM tbl_specialty_tele WHERE `id` = '$ms_id'";
        ...
        $table .='
            ...
            $("#status'.$row['id'].'").change(function(){
                ...
                $.ajax({
                    processData: false,
                    contentType: false,
                    url:"queries/tele-doctor-query.php",
                    type:"POST",
                    data:formData,
...
if (isset($_POST['DeleteDoctor'])) {
    ...
    $first_sql = "SELECT * FROM `tbl_doctor_tele` WHERE `id` = '$id'";
    ...
    $query = "DELETE FROM `tbl_doctor_tele` WHERE `id` = '$id'";
    $result = mysqli_query($conn,$query);
}
...
if (isset($_POST['ChangingStatus'])) {
    $status = $_POST['status'];
    $id = $_POST['id'];
    $sql = "UPDATE `tbl_doctor_tele` SET `status`= '$status' WHERE `id` = '$id'";
